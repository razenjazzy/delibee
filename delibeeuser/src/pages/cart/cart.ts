...
export class CartPage {
  ...
  private cart_qty: number = 0;
  private item_qty: number = 0;
  ...
  decrementItem(product) {
    ...
    if (decremented) {
      ...
      } else this.calculateTotal(false);
    }
  ...
  calculateTotal(couponApplied) {
    ...
    if (couponApplied || this.couponApplied) {
    ...
    if (this.cartItems != null && this.cartItems.length > 0) {
      ...
      if (user != null) {
        if (this.storeSelected && this.total >= this.storeSelected.minimum_order) {
          this.navCtrl.push(ShippingPage, { discount: this.discount });
        } else {
